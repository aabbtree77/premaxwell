...
# Modified to run with Python 3 by Ramunas Girdziusas, 2021.
# Fixed the intersection time bug with Halfspace class.
...
class Scene:
    ...
    def render(self, canvas):
        ...
        halfHeight = (canvas.height/canvas.width) * halfWidth
        width = halfWidth * 2
        height = halfHeight * 2
        pixelWidth = width / (canvas.width - 1)
        pixelHeight = height / (canvas.height - 1)
        #print(f"pixelWidth: {pixelWidth}, pixelHeight: {pixelHeight}")
        ...
        previousfraction = 0
        #print(f"eye.point: {eye.point}, eye.vector: {eye.vector}")
        for y in range(canvas.height):
            ...
            for x in range(canvas.width):
                ...
                #print(f"xcomp: {xcomp}, ycomp: {ycomp}")

        print('Complete.')

    def rayColour(self, ray):
        if self.recursionDepth > 20:
            return (0,0,0)
        try:
